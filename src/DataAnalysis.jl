...
function loedata(data::Tuple{Array, Array}; q=round(Int, length(data[1])/4))
    data[1], loedata(data[1], data[2]; q=q)
end

function loedata(data::Tuple{Array, Array, Array}; q=round(Int, length(data[2])/4), row=false)
    if row
        zt = loedata(data[1], transpose(data[3]); q=q)
        return data[1], data[2], Matrix(transpose(zt))
    else
        return data[1], data[2], loedata(data[2], data[3]; q=q)
    end
end
...
function binstep(V, I, δ)
    binV = collect(V[1]:δ:V[end])
    binVidx = [argmin(abs.(V.-binV[i])) for i in eachindex(binV)]
    Ih = [- -(extrema(I[binVidx[i]:binVidx[i+1]])...) for i in eachindex(binVidx)[1:end-1]]
    binV[2:end], Ih
end
